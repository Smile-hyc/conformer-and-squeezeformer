...
class ConformerASR(nn.Cell):
    ...
    def __init__(self, config, vocab_size):
        ...
        self.ctc_layer = nn.Dense(enc_cfg['d_model'], vocab_size)
        
        # 操作符
        self.equal = ops.Equal()

    def construct(self, audio_feat, audio_len, text_tgt=None):
        """
        前向传播
        audio_feat: 输入音频
        audio_len: 音频长度
        ...
        if text_tgt is not None:      # 生成 Attention Mask (防止看到未来信息)
            ...
            # causal mask(tgt_attn_mask): 下三角矩阵，保证自回归性质，每个位置只能看到之前的信息
            # 形状[target_length, target_length]
            tgt_attn_mask = self.generate_square_subsequent_mask(text_tgt.shape[1])
    ...
    def generate_square_subsequent_mask(self, sz):
        """生成causal mask"""
        # 创建上三角矩阵，k=1表示对角线以上的元素
        mask = mnp.triu(mnp.ones((sz, sz), dtype=mindspore.float32), k=1)
        # 将1的位置填充为负无穷，0的位置保持为0
        mask = ops.masked_fill(mask, self.equal(mask, Tensor(1.0, mindspore.float32)), 
                               Tensor(float('-inf'), mindspore.float32))
        return mask

    def make_pad_mask(self, target):
        """生成 padding mask"""
        # target: [B, L] -> mask: [B, L] (padded的地方是True)
        return self.equal(target, Tensor(0, mindspore.int32))